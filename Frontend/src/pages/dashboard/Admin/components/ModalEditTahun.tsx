...
// Tipe Data Tahun Ajaran dari Props
...
export default function ModalEditTahun({ tahunAjaran, onClose, onRefresh }: ModalEditTahunProps) {
  // State lokal untuk menampung data form (disesuaikan dengan ENUM database)
  const [formData, setFormData] = useState({
    id: "",
    tahun: "",
    status: "AKTIF" // Format ENUM PostgreSQL
  });
  ...
  useEffect(() => {
    if (tahunAjaran) {
      setFormData({
        ...
        // Konversi format UI ke format ENUM Backend
        status: tahunAjaran.status === "Aktif" ? "AKTIF" : "NON_AKTIF"
      });
      setErrorMsg(""); // Reset pesan error jika modal baru dibuka
    }
  }, [tahunAjaran]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`http://187.127.121.139:3000/api/tahun-ajaran/${formData.id}`, {
        method: "PUT",
        headers: {
      ...
      if (result.success) {
        ...
        Swal.fire({
          ...
          text: result.message || "Data tahun ajaran berhasil diperbarui.",
          icon: "success",
        ...
        Swal.fire({
          title: "Gagal Update!",
          text: result.message || "Gagal memperbarui data tahun ajaran.",
          icon: "error",
          confirmButtonColor: "#d33",
        });
        setErrorMsg(result.message || "Gagal memperbarui data tahun ajaran.");
      }
    } catch (error) {
      ...
      <div className="relative bg-white rounded-2xl w-full max-w-md shadow-2xl p-6 md:p-8 animate-fade-in z-10 custom-scrollbar">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            >
              {/* Value disesuaikan dengan ENUM PostgreSQL */}
