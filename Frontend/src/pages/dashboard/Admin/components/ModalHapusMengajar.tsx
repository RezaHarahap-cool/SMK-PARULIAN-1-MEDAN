...
export default function ModalHapusMapel({ mapel, onClose, onRefresh }: ModalHapusMapelProps) {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://187.127.121.139:3000/api/mapel/${mapel.id}`, {
        method: "DELETE",
        headers: {
      ...
      if (result.success) {
        ...
        Swal.fire({
          title: "Terhapus!",
          text: result.message || "Data mata pelajaran berhasil dihapus.",
          icon: "success",
        ...
        Swal.fire({
          title: "Gagal!",
          text: result.message || "Gagal menghapus mata pelajaran.",
          icon: "error",
          confirmButtonColor: "#d33",
        });
        setErrorMsg(result.message || "Gagal menghapus mata pelajaran.");
      }
    } catch (error) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl w-full max-w-md shadow-2xl p-6 md:p-8 animate-fade-in z-10 text-center">
        ...
        {/* Area Pesan Error (Sangat berguna kalau mapel ini terikat foreign key di database) */}
        {errorMsg && (
