...
export default function ModalHapusRoster({ roster, onClose, onRefresh }: ModalHapusRosterProps) {
  ...
  // FUNGSI EKSEKUSI DELETE KE BACKEND
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      // Tembak API dengan method DELETE dan lempar ID Jadwal
      ...
      if (result.success) {
        ...
        Swal.fire({
          title: "Terhapus!",
          text: "Jadwal berhasil dihapus permanen.",
          icon: "success",
        ...
        Swal.fire({
          title: "Gagal!",
          text: result.message || "Gagal menghapus jadwal.",
          icon: "error",
          confirmButtonColor: "#d33",
        });
        setErrorMsg(result.message || "Gagal menghapus jadwal.");
      }
    } catch (error) {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl w-full max-w-md shadow-2xl p-6 md:p-8 animate-fade-in z-10 text-center">
        ...
        <p className="text-gray-500 text-sm mb-4 leading-relaxed">
          Apakah Anda yakin ingin menghapus jadwal <span className="font-bold text-gray-800">"{roster.mapel?.mapel || "Tidak Diketahui"}"</span> untuk kelas <span className="font-semibold text-gray-700">{roster.kelas?.nama_kelas} {roster.kelas?.ruang_kelas ? `- ${roster.kelas.ruang_kelas}` : ""}</span> pada hari <span className="font-semibold text-gray-700">{roster.hari}</span> (Les {roster.les})? 
          <br />
        ...
        {/* Area Pesan Error (Jika server menolak) */}
        {errorMsg && (
