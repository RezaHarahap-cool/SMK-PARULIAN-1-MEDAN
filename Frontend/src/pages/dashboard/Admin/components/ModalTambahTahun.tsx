...
export default function ModalTambahTahun({ isOpen, onClose, onRefresh }: ModalTambahTahunProps) {
  // 1. State Penampung Data Input
  const [tahun, setTahun] = useState("");
  const [status, setStatus] = useState("AKTIF"); // Default ke AKTIF sesuai ENUM
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Tembak API POST ke rute tahun ajaran
      const response = await fetch("http://187.127.121.139:3000/api/tahun-ajaran", {
        ...
        body: JSON.stringify({
          tahun: tahun,
          status: status // Akan mengirim "AKTIF" atau "NON_AKTIF"
        })
      });
      ...
      if (result.success) {
        ...
        Swal.fire({
          title: "Berhasil!",
          text: result.message || "Tahun Ajaran berhasil ditambahkan.",
          icon: "success",
          ...
          if (res.isConfirmed) {
            ...
            onClose(); 
            onRefresh(); // 🔥 OTOMATIS REFRESH TABEL
          }
        });
      } else {
        // 🔥 SweetAlert Error
        Swal.fire({
          title: "Gagal!",
          text: result.message || "Gagal menyimpan data tahun ajaran.",
          icon: "error",
          confirmButtonColor: "#d33",
        });
        setErrorMsg(result.message || "Gagal menyimpan data tahun ajaran.");
      }
    } catch (error) {
      console.error("Error tambah tahun ajaran:", error);
      Swal.fire({
        title: "Server Error",
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <select 
              ...
              className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-black/5 focus:border-black outline-none text-sm bg-white cursor-pointer font-semibold"
            >
              {/* Value disesuaikan persis dengan ENUM StatusTahunAjaran di Prisma */}
              <option value="AKTIF" className="text-green-600">Aktif</option>
              <option value="NON_AKTIF" className="text-red-600">Nonaktif</option>
            </select>
          </div>
