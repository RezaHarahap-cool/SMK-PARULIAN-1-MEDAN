...
// ==========================================
// 1. Tipe Data Profil Siswa (Sesuai response backend)
// ==========================================
interface ProfileDataSiswa {
  username: string;
  email: string;
  role: string;
  siswa?: { // Ganti siswaProfile jadi siswa
    nama_siswa: string;
    foto: string | null;
  };
}
...
export default function Sidebar({ isMobileOpen, setIsMobileOpen }: { isMobileOpen: boolean, setIsMobileOpen: (v: boolean) => void }) {
  ...
  const [profile, setProfile] = useState<ProfileDataSiswa | null>(null);
  ...
  const showSettingPopup = () => {
    ...
    Swal.fire({
      title: "Pengaturan Akun Siswa",
      html: `
        <div style="text-align: left; font-size: 14px;">
          <p style="margin-bottom: 10px;">Pengeditan pas foto, kontak wali, dan alamat tempat tinggal dapat dilakukan langsung pada halaman <b>Profile</b>.</p>
          <div style="padding: 10px; background-color: #e0f2fe; color: #0f172a; border-radius: 8px; border: 1px solid #bae6fd;">
            <b style="font-size: 12px; display: block; margin-bottom: 4px;">📌 INFO PENTING</b>
            Data akademik resmi seperti NIS, NISN, Nama Lengkap, Kelas, dan Status Siswa telah dikunci oleh sistem. Silakan hubungi Tata Usaha jika terdapat kesalahan data.
          </div>
        </div>
      `,
      icon: "info",
  ...
  // Perhatikan relasinya: profile.siswaProfile
  const namaTampil = profile?.siswa?.nama_siswa || profile?.username || "Memuat...";
  const emailTampil = profile?.email || "Menunggu data...";
  
  // Asumsi url fotomu diakses melalui folder uploads di backend
  const fotoTampil = profile?.siswa?.foto 
    ? `http://187.127.121.139:3000/uploads/${profile.siswa.foto}` 
    : null;
  ...
  return (
    <>
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-[#f8f9fa] border-r border-gray-200 flex flex-col transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:h-screen ${isMobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        {/* Footer Sidebar: Profil Siswa & Popup Menu */}
        <div className="p-4 border-t border-gray-200 relative">
          ...
          {showProfileMenu && (
            <>
              ...
              <div className="absolute bottom-[80px] left-4 w-48 bg-white border border-gray-200 rounded-xl shadow-lg py-2 animate-fade-in z-50">
                ...
                <button 
                  onClick={() => { handleMenuClick('/siswa'); setShowProfileMenu(false); }} 
          ...
          {/* Info Siswa (Dinamis dari Backend) */}
          <div 
